...
function loadData() {
    Promise.all([
        d3.csv("data/covid_states_data.csv"),
        d3.csv("data/US_state_names.csv"),
...
function plotBubbleChart(data) {
    ...
    var colorValues = [{ color: '#86ADD4', value: 0 }, { color: '#326FAC', value: maxHospitalized }];
...
function plotBarChart(data, totalData) {
    ...
    barSVG.append('text')
        ...
        .text('*Part of the data is missing')
        .attr('text-anchor', 'middle')
        .attr('fill', '#9A9A9A')
...
function unHighlight(state) {
    ...
    d3.select(`#path-${state}`).attr("stroke", "#C1B4EB");
    // unhighlight bar
